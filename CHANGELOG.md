...
- **`skill-kind: domain` — a project-knowledge layer, so domain detail stops defaulting into always-loaded memory.** The plugin shipped five well-specified plugin-owned skills (`host-handoff`, `host-audit`, `host-doctor`, `sdlc-build-gate`, `code-quality`) and a generic `skill.template.md`, but `SKILL.md`'s Phase 2 plan spec described project skills in exactly one bare bullet — `- Skills to create.` — with no taxonomy, no derivation rule, and no reference, while every other Phase 2 item carried a full specification. No interview step ever produced the list that template would render (§10 asks only about *marketplace lookup*, i.e. skills to **install**), and `skill.template.md` was the one template with **no** `skill-kind` marker at all. The practical consequence: project business rules, regulatory constraints, and a repo's own coordination conventions had nowhere to live except `AGENTS.md` — which is loaded in **every host session** and is the fallback a subagent must open when it needs a rule beyond its compact project-standard digest, so domain detail parked there both taxes every host turn and defeats the v1.6.0 layering by forcing a whole-file load where a scoped skill would do. That fought hard rule #18 and `context-optimization.md` directly, and `skill-format.md`'s `## Progressive Loading` section had nothing routing to it.
  ...
  - **`references/misplaced-artifacts-migration.md`** — a `v1.12.0 → v1.13.0` row that closes a real data-loss risk: project skills generated before this release carry **no** `skill-kind` marker, so `improve`/`upgrade`/`agents-doctor` could not tell them apart from plugin-owned skills and might regenerate a user-authored body. The migration stamps `skill-kind: domain` onto unmarked non-plugin-owned skills, **marker line only**, and routes genuinely ambiguous cases to `manual-review` rather than guessing.

### Changed

- **`SKILL.md` — derivation replaces the bare bullet, with no new question and no new hard rule.** Phase 2's `- Skills to create.` becomes a specified item matching the shape of every other Phase 2 entry: per candidate `name`, trigger, `skill-kind`, and placement-rule justification. Candidates are **derived** from `headline_purpose`, the Phase 1.7 domain classification, the detected stack, Directory Architecture zones carrying non-obvious rules, and existing repo docs/ADRs — explicitly *never* a blank "what skills do you want?" prompt — and are confirmed by Phase 2's **existing** approval gate, exactly as the Directory Architecture and Agent Roster already are. Hard rule **#18** ("Context budget is a feature") is extended in place to carry the placement rule rather than adding a rule #41: a hard rule is the plugin's scarcest namespace (append-only, never renumbered), and #18 already owns context budget. Zero net lines — `SKILL.md` stays at 500.
...
- **`scripts/_validate.py`** — new `check_domain_skill_policy()` guards the marker, the placement rule, the admission gate, the never-overwrite contract, the audit signal, and the interview scope note. Verified by negative test: changing the marker to `skill-kind: project` fails the run.
...
- **`SKILL.md` hard rule #13** — extended in place rather than adding a rule #41. A hard rule is the plugin's scarcest namespace (append-only, never renumbered), and #13 already owns parallelism primitives; the Phase 1.9 Q9e gate folds into the existing Copilot tool-profile item. Zero net lines — `SKILL.md` stays at 500.
- **`scripts/_validate.py`** — new `check_advisory_supervision_policy()` guards the protocol across `parallelism.md`, `handoff.md`, `topology.md`, `interview.md`, `AGENTS.md.template`, and `SKILL.md`, including both release citations. Verified by negative test: reverting `**Polling is banned.**` to weaker wording fails the run.
...
- **Code Quality & Maintainability — authoring craft for the code agents write.** Closes a real gap: the plugin already verified changes *after* they were written (SDLC Build Gate) and governed architecture/security, but it never defined the code-craftsmanship standards a generated coding agent should follow *while writing* — so "Maintainability" existed only as an abstract interview quality-attribute choice. This subsystem makes the code that agents generated by this plugin produce conventional, maintainable, and review-ready, not merely green on CI. It is complementary to the Build Gate (verification) and distinct from `content-quality` (which governs the generated agent *prose*).
  - **New `references/code-quality.md`** — source-backed, original standards (Google Eng-Practices code review, Google Style Guides, ISO/IEC 25010 maintainability, Fowler refactoring/code-smells, Clean Code concepts, Pragmatic Programmer DRY, McCabe cyclomatic complexity, OWASP secure-coding error handling). Leads with **Rule 0 — conform to the project's existing conventions first** (read and obey `.editorconfig`, ESLint/Prettier, Ruff/Black, gofmt, rustfmt, etc.; match local patterns) — the highest-leverage rule. Includes the code-smell signal taxonomy, status rubric, role boundaries, strictness model, propagation contract, and verification.
  - **New `assets/skills/code-quality.skill.md.template`** — host-loaded `code-quality` skill (`skill-kind: code-quality`) emitted at every selected runtime's skills path (Copilot/Claude/OpenCode/Codex/Gemini). The host loads it before any `code-edit`/`bug-fix`/`refactor` assignment; reviewers use it for the maintainability verdict.
  - **New `assets/code-quality-standards.snippet.md`** — compact inline standards (managed block + `{{CODE_QUALITY_STRICTNESS}}`) rendered into `AGENTS.md` › Code Quality & Maintainability (audience: all).
  - **New read-only role `code-quality-reviewer`** — owns the maintainability/convention/code-smell verdict only (distinct from `@reviewer` correctness, `architecture-reviewer` boundaries, `change-bug-hunter` diff-scoped bugs); merges into `@reviewer` for `light`/`advisory`/tiny setups.
  - **Propagation contract** — so the standards actually reach the code-writing agent: every subagent template (Copilot/Claude/OpenCode/Gemini/Codex) carries both an imperative to **apply** the Code Quality & Maintainability standards while editing/reviewing *and* a `Code quality: ok|warn|fail|n/a; signals=<list|none>` reporting marker; the `task-handoff` skill loads `code-quality` for `code-edit`/`bug-fix`/`refactor`/`code-review` tasks and the host passes `Skills Referenced: code-quality loaded=true`. The OpenCode root-session skill gate allowlists `code-quality` alongside `task-handoff`.
  - **Strictness decoupled from the Build Gate** — `code_quality_strictness ∈ {standard, strict, light, advisory, skipped, n/a}`: mirrors `build_gate_strictness`, floors to `light` when the gate is skipped, uses `advisory` for non-software-dev but code-bearing repos (scripts/IaC/notebooks), and `n/a` only when there is no source code — so "skip the Build Gate" never silently means "skip clean code."

### Changed

- **`SKILL.md` (499 → 500 lines):** new hard rule **#40** (code quality & maintainability is mandatory for software-dev; conventions-first; emit the `code-quality` skill, AGENTS.md section, and `code-quality-reviewer`; pass the loaded-skill evidence). Phase 1.9, Phase 2, Phase 4 emission, and Phase 7 verification gain code-quality wiring co-located with the Build Gate lines.
- **`scripts/_validate.py`:** new `check_code_quality_policy()` asserts the reference, skill template, snippet, `AGENTS.md.template` placeholders, SKILL.md hard rule + wiring, topology role/sizing, interview seeding, the Build-Gate cross-link, and the `Code quality:` propagation markers across all five subagent templates plus `task-handoff`.
- **`assets/AGENTS.md.template`:** new `## Code Quality & Maintainability` section + a Project Snapshot `Code quality` status line.
- **`references/topology.md`:** `code-quality-reviewer` row + a Code Quality Sizing Rule.
- **`references/interview.md`:** Q9d now also seeds `code_quality_strictness` (no second blocking question — decoupled so "skip the Build Gate" never means "skip clean code").
- **`references/sdlc-build-gate.md`:** the Code review gate cross-links the code-quality maintainability verdict.

### Validation

- `bash scripts/validate.sh`: `[OK] All checks passed (2 warning(s))` — SKILL.md is now 500 lines (at the hard cap, warn-only) and the Codex `developer_instructions` budget warning grows by one line from the added `Code quality:` marker.
- `npx --yes markdownlint-cli2@0.22.1 "**/*.md"`: 51 files, 0 errors.
- Regression check: removing the `Code quality:` marker from a subagent template, or a required marker from the reference/snippet/skill, trips `check_code_quality_policy()`; restoring it returns a clean run.
