...
| `v1.9.0 → v1.13.0` | Orchestrator-side only, and only for systems where **Copilot CLI/app is a selected runtime**. Covers every `v1.9.0` / `v1.10.x` / `v1.11.0` stamp — `v1.12.0` was developed but never published, so no generated system carries that stamp. The two Copilot-app advisory lines in `AGENTS.md` — the `### Platform-native delegation` Copilot row note and the `## Wave Execution` bullet — were introduced by v1.11.0, so a system stamped `v1.9.0` / `v1.10.0` / `v1.10.1` does **not** have them: **add them first** (that is v1.11.0's unshipped migration delta, folded in here), then extend both with the supervision clause and the fail-closed `n/a` fallback. Do **not** add a new `AGENTS.md` section, do not insert a heading between `### Platform-native delegation` and `## Wave Execution`, and do not mutate any subagent file. Default `advisory_supervision = off`: unless the user opts in during the upgrade interview, the supervision clause is inert prose and no behavior changes. Skip entirely for non-Copilot systems. The `parallel_safe_units < 3` floor suppresses only the **supervision** clause — the v1.11.0 dispatch advisory has no sizing floor and is still added. |
| `pre-v1.13.0 → v1.13.0` | **Marker-only, and it prevents a real data-loss risk.** Project skills generated from `skill.template.md` before v1.13.0 carry **no** `skill-kind` marker, so `improve` / `upgrade` / `agents-doctor` cannot distinguish them from plugin-owned skills and could regenerate a user-authored body. For every skill at a runtime skills path whose `skill-kind` is absent **and** whose name is not one of the five plugin-owned kinds (`task-handoff`, `tool-catalog-audit`, `agents-doctor`, `code-change-build-gate`, `code-quality`), insert `<!-- agents-system-setup:skill-kind: domain -->` after the existing `generated-at` marker. **Touch the marker line only — never the body, headings, or frontmatter.** If a skill's ownership is genuinely ambiguous (name collides with a plugin-owned kind but content is clearly project-specific), record it as `manual-review` and ask; do not guess. Then re-run the placement rule over `AGENTS.md`: knowledge that only *some* tasks need is a `domain`-skill candidate, but propose the move — never apply it silently. |
