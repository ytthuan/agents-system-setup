...
If the user picked **improve** → run the [improve procedure](./references/replication.md#4-improve-mode-audit--targeted-upgrade) (audit → score → propose deltas → opt-in apply). Misplaced artifacts detected in Phase 1 are first-class deltas; surface them through [misplaced-artifacts-migration](./references/misplaced-artifacts-migration.md) before any Phase 5 write. Skip Phases 2–4; jump to Phase 5 mechanics for backups + write.

If the user picked **upgrade** → run the [version stamp detection & migration playbook](./references/misplaced-artifacts-migration.md#version-stamp-detection--migration-playbook): read `.agents-system-setup/generated.json` (or per-file `generated-by` stamps), compare against `plugin.json` version, walk the per-version delta table in order, ask once before each migration, back up before each rewrite, preserve user customizations outside managed blocks, then update the manifest atomically. Skip Phases 2–4; jump to Phase 5. Triggers: `agents-system-setup upgrade`, `/setup-copilot-agents upgrade`, `$agents-system-setup upgrade` (Codex), or host-runtime equivalent. Non-destructive and reversible via `.bak`.
